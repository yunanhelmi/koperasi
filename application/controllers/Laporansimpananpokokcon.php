...
class LaporansimpananpokokCon extends CI_Controller {
    ...
    function html() {
        ...
        $simpananpokok = $this->laporansimpananpokokmodel->get_data_simpananpokok($tanggal);
        ...
        $data['data'] = $simpananpokok;
        $data['total_neraca'] = $total_neraca;

        $this->load->view('/hasil_laporan/simpananpokok', $data);
    }
	...
	function excel() {
        ...
        $i++;
        $sheet->mergeCells("A".$i.":J".$i)->setCellValue("A".$i, "LAPORAN DAFTAR SIMPANAN POKOK ".$tanggal1);
        $sheet->getStyle("A".$i.":J".$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
