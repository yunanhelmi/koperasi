...
class LaporansimpanankhususCon extends CI_Controller {
	function __construct() {
		...
		$this->load->model('laporansimpanankhususmodel');
	...
	function index() {
		...
		$this->load->view('/laporan/simpanankhusus', $data);
    ...
    function html() {
        ...
        $simpanankhusus = $this->laporansimpanankhususmodel->get_data($tanggal);

        // Total Simpanan Khusus pada neraca
        if($tanggal < '2019-01-01') {
            $transaksi    = $this->transaksiakuntansimodel->get_jumlah_by_sampai_kode_akun($tanggal, '303');
        } else if($tanggal >= '2019-01-01') {
            $transaksi    = $this->transaksiakuntansimodel->get_jumlah_by_dari_sampai_kode_akun('2019-01-01', $tanggal, '303');
        }
        $total_neraca = $transaksi[0]['jumlah_kredit'] - $transaksi[0]['jumlah_debet'];

        $data['tanggal'] = $tanggal;
        $data['data'] = $simpanankhusus;
        $data['total_neraca'] = $total_neraca;

        $this->load->view('/hasil_laporan/simpanankhusus', $data);
    }
	...
	function excel() {
		...
		$data = $this->laporansimpanankhususmodel->get_data($tanggal);
        ...
        $file->getProperties ()->setTitle ( "Laporan Daftar Simpanan Khusus" );
        $file->getProperties ()->setSubject ( "Laporan Daftar Simpanan Khusus" );
        $file->getProperties ()->setDescription ( "Laporan Daftar Simpanan Khusus" );
        $file->getProperties ()->setKeywords ( "Laporan Daftar Simpanan Khusus" );
        $file->getProperties ()->setCategory ( "Laporan Daftar Simpanan Khusus" );
