...
class LaporansimpananpihakketigaCon extends CI_Controller {
	function __construct() {
		...
		$this->load->model('laporansimpananpihakketigamodel');
	...
	function index() {
		...
		$this->load->view('/laporan/simpananpihakketiga', $data);
    ...
    function html() {
        ...
        $simpananpihakketiga = $this->laporansimpananpihakketigamodel->get_data($tanggal);

        // Total Simpanan Pihak Ketiga pada neraca
        if($tanggal < '2019-01-01') {
            $transaksi    = $this->transaksiakuntansimodel->get_jumlah_by_sampai_kode_akun($tanggal, '227');
        } else if($tanggal >= '2019-01-01') {
            $transaksi    = $this->transaksiakuntansimodel->get_jumlah_by_dari_sampai_kode_akun('2019-01-01', $tanggal, '227');
        }
        $total_neraca = $transaksi[0]['jumlah_kredit'] - $transaksi[0]['jumlah_debet'];

        $data['tanggal'] = $tanggal;
        $data['data'] = $simpananpihakketiga;
        $data['total_neraca'] = $total_neraca;

        $this->load->view('/hasil_laporan/simpananpihakketiga', $data);
    }
	...
	function excel() {
		...
		$data = $this->laporansimpananpihakketigamodel->get_data($tanggal);
        ...
        $file->getProperties ()->setTitle ( "Laporan Daftar Simpanan Pihak Ketiga" );
        $file->getProperties ()->setSubject ( "Laporan Daftar Simpanan Pihak Ketiga" );
        $file->getProperties ()->setDescription ( "Laporan Daftar Simpanan Pihak Ketiga" );
        $file->getProperties ()->setKeywords ( "Laporan Daftar Simpanan Pihak Ketiga" );
        $file->getProperties ()->setCategory ( "Laporan Daftar Simpanan Pihak Ketiga" );
        ...
        $i++;
        $sheet->mergeCells("A".$i.":J".$i)->setCellValue("A".$i, "LAPORAN DAFTAR SIMPANAN PIHAK KETIGA ".$tanggal1);
        $sheet->getStyle("A".$i.":J".$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        $filename = "Laporan Daftar Simpanan Pihak Ketiga_".$tanggal1.".xlsx";
