...
class LaporansimpananwajibCon extends CI_Controller {
    ...
    function html() {
        ...
        $simpananwajib = $this->laporansimpananwajibmodel->get_data($tanggal);
        ...
        $data['data'] = $simpananwajib;
        $data['total_neraca'] = $total_neraca;

        $this->load->view('/hasil_laporan/simpananwajib', $data);
    }
	...
	function excel() {
        ...
        $i++;
        $sheet->mergeCells("A".$i.":J".$i)->setCellValue("A".$i, "LAPORAN DAFTAR SIMPANAN WAJIB ".$tanggal1);
        $sheet->getStyle("A".$i.":J".$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
